...
function GooogleSpreadsheet( ss_key, auth_id, options ){
  ...
  this.getRows = function( worksheet_id, opts, cb ){
    ...
    self.makeFeedRequest( ["list", ss_key, worksheet_id], 'GET', query, function(err, data, xml) {
      ...
      entries.forEach( function( row_data ) {
        rows.push( new Row( self, row_data, entries_xml[ i++ ] ) );
      });
      cb(null, rows);
    });
  }
  ...
  this.bulkUpdateCells = function (worksheet_id, cells, cb) {
    ...
    var worksheetUrl = GOOGLE_FEED_URL+"cells/" + ss_key + "/" + worksheet_id + "/private/full";
    var data_xml =
    [
    ...
    self.makeFeedRequest(worksheetUrl+"/batch", 'POST', data_xml, cb);
  };
};
