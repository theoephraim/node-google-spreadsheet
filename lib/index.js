...
function GooogleSpreadsheet( ss_key, auth_id, options ){
  ...
  this.makeFeedRequest = function( url_params, method, query_or_data, cb ){
    ...
    async.series({
      ...
      request: function(result, step) {
        ...
        // [Hack] `bulkUpdateCells()` and update worksheets` don't work well
        // with etags, so we ignore them
        var us = url.split('/')
        if(us[us.length-1] === 'batch' || us[4] === 'worksheets')
          headers['If-Match'] = '*'
        ...
        function(err, response, body)
        {
          ...
          if(statusCode >= 400)
            return cb( new Error("HTTP error "+statusCode+": "+http.STATUS_CODES[statusCode])+" "+JSON.stringify(body))
          if(statusCode === 200 && response.headers['content-type'].indexOf('text/html') > -1)
  ...
  this.addRow = function( worksheet_id, data, cb ){
    ...
    .map(function(key)
    {
      var colName = xmlSafeColumnName(key)

      return '<gsx:'+colName+'>'+xmlSafeValue(data[key])+'</gsx:'+colName+'>'
    })
