...
class GoogleSpreadsheetWorksheet {
  ...
  async clear(cellRange) {
    // clears all the data in the sheet
    // sheet name without ie 'sheet1' rather than 'sheet1'!A1:B5 is all cells
    const sheetName = cellRange ? `${this.encodedA1SheetName}!${cellRange}` : this.encodedA1SheetName;
    await this._spreadsheet.axios.post(`/values/${sheetName}:clear`);
