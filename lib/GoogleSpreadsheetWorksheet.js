...
class GoogleSpreadsheetWorksheet {
  ...
  get headerRowIndex() {return this.getheaderRowIndex()}
  ...
  set columnCount(newVal) { throw new Error('Do not update directly. Use resize()'); }
    set headerRowIndex(newVal) {
    throw new Error('Do not update directly. Use getRows({headerRowIndex: value })')
  }
  ...
  setheaderRowIndex(newVal) {
    this._headerRowIndex = newVal;
  }
  ...
  async loadHeaderRow() {
    const a1Index = this._headerRowIndex + 1 || 1;
  ...
  async getRows(options = {}) {
    ...
    options.skipEmpty = options.skipEmpty || false;
    this.setheaderRowIndex(options.headerRowIndex);
    if (!this.headerValues) await this.loadHeaderRow();
