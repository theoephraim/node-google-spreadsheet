...
class GoogleSpreadsheetWorksheet {
  ...
  async loadHeaderRow(row) {
    this._headerRowIndex = row || this._headerRowIndex;
  ...
  async getRows(options = {}) {
    ...
    options.headerRowIndex = options.headerRowIndex || 1;

    if (!this.headerValues) await this.loadHeaderRow(options.headerRowIndex);

    const firstRow = options.headerRowIndex + 1 + options.offset; // start below header
