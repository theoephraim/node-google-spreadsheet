...
class GoogleSpreadsheetWorksheet {
  ...
  /*
   * Get a range represented as an array of rows with cells.
   */
  getRangeByName(a1Address) {
    const split = a1Address.match(/([A-Z]+)([0-9]+)(?::([A-Z]+)([0-9]*))?/);
    const startColumnIndex = letterToColumn(split[1]) - 1;
    const startRowIndex = parseInt(split[2]) - 1;
    const endColumnIndex = split[3] ? letterToColumn(split[3]) - 1 : startColumnIndex;
    const endRowIndex = split[3] ? (split[4] ? parseInt(split[4]) - 1 : this.rowCount - 1) : startRowIndex;

    // Is start or end address out of range?
    if (
      startRowIndex < 0
      || startColumnIndex < 0
      || endColumnIndex < 0
    ...
    if (
      startRowIndex >= this.rowCount
      || startColumnIndex >= this.columnCount
      || endRowIndex >= this.rowCount
      || endColumnIndex >= this.columnCount
    ) {
      throw new Error(`Out of bounds, sheet is ${this.rowCount} by ${this.columnCount}`);
    }

    // Create range from cells.
    const range = [];
    for (let indexRow = startRowIndex; indexRow <= endRowIndex; indexRow++) {
      const row = this._cells[indexRow];
      if (!row) {
        throw new Error(`The row '${startRowIndex}' has not been loaded yet.`);
      }

      const rangeRow = [];
      for (let indexColumn = startColumnIndex; indexColumn <= endColumnIndex; indexColumn++) {
        const cell = row[indexColumn];
        if (!cell) {
          throw new Error(`The cell '${startRowIndex}:${indexColumn}' has not been loaded yet.`);
        }
