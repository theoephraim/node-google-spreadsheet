...
Worksheet.prototype.setHeaders = function(colnames, cb)
{
  var self = this
  ...
  function updateHeaders()
  {
    ...
    self.bulkUpdateCells(colnames, function(error, result, body)
    {
  ...
  if(length <= this.colCount) return updateHeaders()

  this.setColumns(length, function(error, result)
  {
    if(error) return cb(error)
