...
class GoogleSpreadsheet {
  ...
  constructor(sheetId) {
    ...
    this.axios = Axios.create({
      baseURL: `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}`, // send arrays in params with duplicate keys - ie `?thing=1&thing=2` vs `?thing[]=1...`
  ...
  async create(title) {
    const { data } = await this.axios.post(this.url, { properties: { title } });
  ...
  async addSheet(properties = {}) {
    ...
    if (properties.headerValues || properties.headers) {
      await newSheet.setHeaderRow(
        properties.headerValues || properties.headers
      );
    }
